...
def start():
    ...
    if 'face_recognition_model.pkl' not in os.listdir('static'):
        names, rolls, times = extract_attendance()
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if cv2.waitKey(1) == ord('a'):
                ...
                print(
                    f"attendence marked for {identified_person}, at {current_time_} ")
        ...
        cv2.imshow(
            'Attendance Check, press "q" to exit, press "a" to mark attendence ', frame)
        cv2.putText(frame, 'hello', (30, 30),
                    cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 255))

    # Wait for the user to press 'q' to quit
        if cv2.waitKey(1) == ord('q'):
            names, rolls, times, l = extract_attendance()
            MESSAGE = 'Exited without taking attendance'
            return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(), datetoday2=datetoday2, mess=MESSAGE)
            break
